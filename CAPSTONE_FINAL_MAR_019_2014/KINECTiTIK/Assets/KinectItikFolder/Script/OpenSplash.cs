...
public class OpenSplash : MonoBehaviour
{
	...
	void Start()	
	{
			...
			gestureRect = new LTRect(Screen.width / 2 - 995, Screen.height / 2 - 1005, 100, 25);
			// Slide in
			Vector2 orig = new Vector2( gestureRect.rect.x, gestureRect.rect.y );
			//LeanTween.move( gestureRect, new Vector2( 0.3f*Screen.width - slogan.width, 0.26f*Screen.height ), 1.0f, new object[]{ "ease", LeanTweenType.easeOutBounce } );
		 	//Bounce--->
			//LeanTween.move( gestureRect, orig, 1.0f, new object[]{ "delay", 1.0f, "ease", LeanTweenType.easeOutBounce } );	
			//--bounce
	...
	{
		GUI.depth = guiDepth;
	//	GUI.DrawTexture( gestureRect.rect, slogan);
